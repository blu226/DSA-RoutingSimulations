...
def run_simulation(DataSet, Day_Or_NumMules, Round, Protocol, Band, t, ts, v, generate_LE, Max_Nodes, pkl_fold_num,
                   perfect_knowledge, src_dst, speed, num_mes, num_chan, num_puser, smart_setting, num_fwd, msg_round,
                   puser_round, msg_mean, ttl, max_mem, replicas, priority_queue_active, routing_opt, wei_param,
                   num_trans):
    ...
    if dataset == "UMass":
        ...
        path_to_metrics = link_exists_path + protocol + "/" + buffer + "/" + num_replicas_val + "/mules_" + str(
        ...
        if pkl_fold_num == 1:
            ...
            path_to_day1_LLC = dataMule_path + "Link_Exists/LE_" + str(start_time - T) + "_" + str(
    ...
    create_constants(T, V, S, start_time, dataset, max_nodes, dataMule_path, path_to_folder, path_to_metrics,
                     link_exists_path,
                     debug_message, protocol, NoOfDataCenters, NoOfSources, generate_LE, generate_messages,
                     num_messages,
                     pkl_fold_num, path_to_day1_LLC, perfect_knowledge, speed, limited_time_to_transfer,
                     restrict_band_access,
                     restrict_channel_access, generate_new_primary_users, num_chan, num_puser, path_to_save_LLC,
                     smart_setting,
                     priority_queue_active, broadcast, geo_routing, num_nodes_to_fwd, msg_round, puser_round,
                     debug_mode, metric_interval,
...
def run_various_sims_geo(sim_round, num_mules, num_channels, num_Pusers, msg_round, msg_mean, ttl, mem_size,
    ...
    for band in bands:
        print("Geo: Msg round", msg_round, "PU round", puser_round, " No. Replica", num_replicas, "Mem", mem_size)
...
def run_various_sims(sim_round, num_mules, num_channels, num_Pusers, msg_round, msg_mean, ttl, mem_size, num_replicas,
                     puser_round, wei_param, num_transceivers):
    for band in ["ALL"]:
        print(band, "Msg round", msg_round, "PU round", puser_round, "Mem", mem_size)
                    ...
                    for wei_param in [0.5]:
                        ...
                        for num_replicas in [5]:  # 1, 5
                            run_various_sims_geo(sim_round, num_mules, num_channels, num_Pusers, msg_round,
                                                 msg_mean, ttl, mem_size, num_replicas, puser_round, wei_param,
                                                 num_transceivers)
