...
num_mules = 10
num_channels = 6
num_Pusers = [x for x in range(0, 101, 25)]
T = 180
startTime = 840
num_messages = 75
days = ["2007-11-06"]
...
for i in range(len(days)):
    for protocol in protocols:
        for buffer_type in buffer_types:
            t = 0
            for num_puser in num_Pusers:

                path = "DataMules/UMass/" + days[i] + "/1/Link_Exists/LE_" + str(startTime) + "_" + str(T) + "/" + protocol + "/" + buffer_type + "/mules_" + str(num_mules) + "/channels_" + str(num_channels) + "/P_users_" + str(num_puser) + "/" + str(num_messages) + "/"
...
x = np.array([x for x in range(0, 101, 25)])
plt.xticks(fontsize=20)
plt.yticks(fontsize=25)
plt.xticks(np.arange(0, 101, 25))
title_str = "Time: " + str(T) + "    Channels: " + str(num_channels) + "    Messages: " + str(num_messages)
plt.title(title_str)
# plt.xlim(0,12)
fig_name = "dummy.eps"
...
if p_id == 2:
    ...
    fig_name = "Plots/latency_PU_SER.eps"

if p_id == 3:
    plt.ylabel('Energy expenditure (KJ)', fontsize=25)
    plt.xlabel('Number of Primary Users', fontsize=25)
    plt.ylim(-1, 10)
    fig_name = "Plots/energy_PU_SER.eps"

if p_id == 4:
    plt.ylabel('Message overhead', fontsize=25)
    plt.xlabel('Number of Primary Users', fontsize=25)
    plt.ylim(-1, 37)
    fig_name = "Plots/overhead_PU_SER.eps"
...
if p_id == 1:
    ...
    plt.legend(["Optimistic (FIFO)", "Pessimistic (FIFO)", "Random (FIFO)", "Optimistic (PQ)", "Pessimistic (PQ)", "Random (PQ)"], loc="center left", fontsize=15, ncol = 1, frameon=False)
elif p_id ==4:
