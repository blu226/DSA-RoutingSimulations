...
for i in range(len(days)):
    for protocol in protocols:
        for buffer_type in buffer_types:
            t = 0
            for Puser in num_Pusers:
                for num_fwd in ["1", "5", "10"]:

                    path = "DataMules/Lexington/" + days[i] + "/1/Link_Exists/LE_" + str(startTime) + "_" + str(T) + "/" + protocol + "/" + buffer_type + "/geo_" + num_fwd + "/mules_" + str(num_mules) + "/channels_" + str(num_channels) + "/P_users_" + str(Puser) + "/" + str(num_messages) + "/"
...
if p_id == 1:
    ...
    plt.ylim(-0.05,1.1)
    fig_name = "Plots/pdr_PUser_SER.png"
...
if p_id == 2:
    ...
    fig_name = "Plots/latency_PUser_SER.png"

if p_id == 3:
    plt.ylabel('Energy expenditure (KJ)', fontsize=25)
    plt.xlabel('Number of primary users', fontsize=25)
    plt.ylim(-1, 17)
    fig_name = "Plots/energy_PUser_SER.png"

if p_id == 4:
    plt.ylabel('Message overhead', fontsize=25)
    plt.xlabel('Number of primary users', fontsize=25)
    plt.ylim(-1, 20)
    fig_name = "Plots/overhead_PUser_SER.png"
